...
def get_city_data(twtr_client, ctyjson):
    ...
    city_data_list = []
    for city in ctyjson:
        city_data = CityData(city["city"], long=float(city["lng"]), lat=float(city["lat"]), population=int(city["population"]))
...
class UpdateCityData:
    def __init__(self, ax, cd_list, mapfile):
        ...
        self.ax_time.legend()
        self.timestamp = self.ax_time.axvline(cd_list[0].times[10], linestyle='--', color='black')
        ...
        for c in cd_list:
            ...
            self.ax_map.annotate(c.name, (c.long+0.1, c.lat+0.1), fontsize=8)

        self.count_scale_factor = 3
        # scale by population and a factor for visibility
        self.tweet_activity = self.ax_map.scatter([c.long for c in cd_list], [c.lat for c in cd_list],
                                                   color="red", 
                                                   s=[(3_000_000/c.population)*(c.counts[0]/self.count_scale_factor) for c in cd_list], 
                                                   alpha=0.3)
    ...
    def __call__(self, i):
        ...
        # scale by population and a factor for visibility
        self.tweet_activity.set_sizes([(3_000_000/c.population)*(c.counts[i] / self.count_scale_factor) for c in self.cd_list])
